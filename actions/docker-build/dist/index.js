...
var require_artifact_handler = __commonJS({
  "artifact-handler.js"(exports, module2) {
    ...
    var downloadArtifactAsync = async (key) => {
      ...
      core2.info(`Artifact ${downloadResponse.artifactName} was downloaded to ${downloadResponse.downloadPath}`);
    };
    var uploadArtifactAsync = async (key, path2) => {
      ...
      if (uploadResponse.failedItems.length > 0) {
        core2.setFailed(`An error was encountered when uploading ${uploadResponse.artifactName}. There were ${uploadResponse.failedItems.length} items that failed to upload.`);
        return false;
      }
      core2.info(`Artifact ${uploadResponse.artifactName} has been successfully uploaded!`);
      return true;
    };
    module2.exports = {
      loadGitStateAsync: async () => {
        ...
        core2.info("read git state contents: " + fileContents);
        return JSON.parse(fileContents);
      },
      storeImageNameAndTagAsync: async (fqImageName, imageTag) => {
        const imageTagFile = core2.getInput(constants.inputImageName);
...
var require_image_namer = __commonJS({
  "image-namer.js"(exports, module2) {
    ...
    module2.exports = {
      loadFqImageName: () => {
        let registry = core2.getInput("registry").trim();
        if (registry != "" && !registry.endsWith("/")) {
          registry += "/";
        }
        if (process.env.ACT === "true") {
          core2.info("Setting container registry to empty string for local container builds");
          registry = "";
        }
        return `${registry}${process.env.STACK_NAME}/${core2.getInput("image_name")}`;
      },
      generateImageTag: (gitState) => {
        let imageTag = gitState.commitSha.substring(0, 7);
        if (process.env.ACT === "true") {
          core2.info("Adding unix epoch to image tag for local builds");
